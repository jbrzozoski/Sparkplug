...
import com.cirruslink.sparkplug.message.model.Message.MessageBuilder;
...
public class MessageUtil {
	 ...
	 * maximum size.
	 */
	private static void divideAndAddMessages(Collection<Message> messages, Message message, int maxBytes, String p)
			throws SparkplugException, JsonProcessingException {
		...
		if (size <= 1) {
			throw new SparkplugException("Cannot divide SparkplugBPayload with only " + size + " metrics");
		}
		...
		while (index < metricCount) {
			...
			if (jsonMessage.getBytes().length < maxBytes) {
				...
				divideAndAddMessages(messages, newMessage, maxBytes, p + p);
			}
...
/*	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < numOfMetrics; i++) {
			...
			String str = "";
			for (int j = 0; j < r; j++) {
				str = str + ONE_THOUSAND_BYTE_STRING;
			}
		...
		Message message = new MessageBuilder()
				.topic(TopicUtil.parseTopic("spBv1.0/Group/DBIRTH/My Test Edge/My Device"))
