...
public class Connection {
    ...
    private final Map<String, Metric> sparkplugMetrics = new HashMap<>();
    ...
    public void addSparkplugMetrics(Map<String, Metric> metrics) {
        sparkplugMetrics.putAll(metrics);
    }

    public Map<String, Metric> getSparkplugMetrics() {
        return sparkplugMetrics;
    }
    ...
    public SparkplugBPayloadBuilder addBdSeqNum(SparkplugBPayloadBuilder payload) throws Exception {
        ...
        payload.addMetric(new MetricBuilder("bdSeq", MetricDataType.Int64, (long)bdSeq).createMetric());
        bdSeq++;
        return payload;
    }

    public long getBdSeqNum() {
        return bdSeq;
    }
    ...
    public void publishBirth(Activity activity) {
        try {
            synchronized(getSeqLock()) {
                // Reset the sequence number
                Log.d(TAG, "Publishing Birth");
                resetSeqNum();
                ...
                payload.addMetric(new MetricBuilder("bdSeq", MetricDataType.Int64, (long)getBdSeqNum()).createMetric());
                payload.addMetric(new MetricBuilder("Node Control/Rebirth", MetricDataType.Boolean, false).createMetric());

                for (Metric metric : getSparkplugMetrics().values()) {
                    Log.d(TAG, "Adding metric: " + metric.getName());
                ...
                // Set up the callback
                String topic = "spBv1.0/" + getGroupId() + "/NBIRTH/" + getEdgeNodeId();
                byte[] bytePayload = encoder.getBytes(payload);
                if (activity != null) {
                    String[] actionArgs = new String[1];
                    actionArgs[0] = getId();
                    final ActionListener callback = new ActionListener(activity,
                            ActionListener.Action.PUBLISH, this, actionArgs);

                    getClient().publish(topic, bytePayload, 0, false, null, callback);
                    messageHistory.add(0, new PublishedMessage(topic, new MqttMessage(bytePayload)));
                    HistoryFragment.notifyDataSetChanged();
                } else {
                    getClient().publish(topic, bytePayload, 0, false);
                    messageHistory.add(0, new PublishedMessage(topic, new MqttMessage(bytePayload)));
                    HistoryFragment.notifyDataSetChanged();
                }
            }
    ...
    private class Publisher implements Runnable {
        ...
        public Publisher(String topic, SparkplugBPayload outboundPayload, ActionListener callback) {
            this.topic = topic;
        ...
        public void run() {
            try {
                ...
                SparkplugBPayloadEncoder encoder = new SparkplugBPayloadEncoder();
                byte[] bytePayload = encoder.getBytes(outboundPayload);
                if (callback != null) {
                    client.publish(topic, bytePayload, 0, false, null, callback);
                    messageHistory.add(new PublishedMessage(topic, new MqttMessage(bytePayload)));
                } else {
                    client.publish(topic, bytePayload, 0, false);
                    messageHistory.add(new PublishedMessage(topic, new MqttMessage(bytePayload)));
                }
    ...
    private Location getGpsLocation() {
        if (MainActivity.HAS_LOCATION_PERMISSION) {
            LocationManager locationManager = (LocationManager) context.getSystemService(Context.LOCATION_SERVICE);

            //exceptions will be thrown if provider is not permitted.
            try {
                Log.d(TAG, "Native GPS Enabled? " + locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER));

                // Seed for the emulator
                locationManager.requestSingleUpdate(LocationManager.GPS_PROVIDER, new LocationListener() {
                    @Override
                    public void onLocationChanged(Location location) {
                        Log.d(TAG, "Native onLocationChanged");
                    }

                    @Override
                    public void onStatusChanged(String s, int i, Bundle bundle) {
                        Log.d(TAG, "Native onStatusChanged");
                    }

                    @Override
                    public void onProviderEnabled(String s) {
                        Log.d(TAG, "Native onProviderEnabled");
                    }

                    @Override
                    public void onProviderDisabled(String s) {
                        Log.d(TAG, "Native onProviderDisabled");
                    }
                }, null);
            } catch (Exception e) {
                Log.e(TAG, "Native GPS not enabled ", e);
            }

            try {
                Log.d(TAG, "Network GPS Enabled? " + locationManager.isProviderEnabled(LocationManager.NETWORK_PROVIDER));
                ...
                Log.e(TAG, "Network GPS not enabled ", e);
            }
            ...
            if (null != locationGPS) {
                Log.d(TAG, "Native GPS Location: " + locationGPS.getLatitude() + " :: " + locationGPS.getLongitude());
                GPSLocationTime = locationGPS.getTime();
            } else {
                Log.d(TAG, "Native GPS Location is null");
            }

            long NetLocationTime = 0;

            if (null != locationNet) {
                Log.d(TAG, "Network GPS Location: " + locationNet.getLatitude() + " :: " + locationNet.getLongitude());
                NetLocationTime = locationNet.getTime();
            } else {
                Log.d(TAG, "Network GPS Location is null");
            }
